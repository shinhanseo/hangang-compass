...
- 서울시 실시간 혼잡 API를 5분 서버 캐시와 도착 시각 예측 선택 규칙을 거쳐 추천 점수·결과 화면에 연결
...
- 참여자별 대중교통·자가용 선택과 `주차장까지 운전 + 공통 집결점까지 도보` 실제 경로를 같은 공평성 규칙으로 비교
- 카카오 키워드 검색으로 역·학교·건물·공공장소를 자유롭게 찾아 출발 장소로 선택하는 흐름 연결
- 방장이 이동 패턴과 공통 출발 장소를 정하고 참여자의 갈 때·귀가 이동 공평성을 비교하는 흐름 연결
- 방장 자신의 출발·귀가 장소를 추천 참여자로 제출한 뒤 친구를 초대하는 흐름 연결
- 최대 8명 참여 안내와 카카오톡 우선 초대·링크 복사 대안 연결
- 추천 결론과 이동·공평성·해당 공원 혼잡을 먼저 보여주고, 대안 뒤에서 11개 공원의 현재·도착 예상 혼잡을 비교하는 그래프 연결
- 약속 날짜·시간을 앱 팔레트의 모바일 바텀시트에서 선택하는 흐름 연결
- 경로 실패 시 fake 시간으로 대체하지 않는 장애 상태, 공급자별 일일 한도 차단·사용량 원장, SQLite/PostgreSQL 공용 경로 캐시와 추천 결과 캐시 구현
- 로컬 SQLite·배포용 Supabase PostgreSQL 약속 저장소, 같은 기기의 진행 중인 약속, 기기 간 방장 전용 복구 링크 구현
- 방장 확인 바텀시트를 거친 약속 취소와 참여 정보·초대 권한의 즉시 삭제 구현
- 추천 3곳을 그대로 쓰는 선택형 친구 투표, 카카오 공유, 동률 재투표·명시적 랜덤 선택 구현
...
- 친구들과의 한강 피크닉
- 참여자별 출발역 또는 장소 입력
- 함께 출발·각자 귀가 또는 각자 출발·각자 귀가 패턴
- 대중교통·자가용 혼합 이동 공평성 비교
- 추천 공원 1곳과 성격이 다른 대안 2곳
- 공원 내부의 정확한 만남 지점
- 도착 시점 혼잡·날씨·행사 근거
- 공원별 피크닉 특징·대표 즐길거리·이용 제한
- 확정 공원의 만남 장소·즐길거리 안내와 실제 주변 명소·음식점·카페·편의점 거리순 추천
- 설치와 필수 회원가입 없는 링크 참여

혼잡 예측 범위 밖의 약속에는 이동 기준 1차 추천과 `오늘 지금 기준` 혼잡 그래프를 분리해 제공합니다. 약속 12시간 이내에 들어오면 열린 결과 화면이 도착 혼잡 예측을 자동 반영하고 추천 유지·변경 여부를 알립니다. 추천 장소가 달라져도 방장 승인 없이 확정 장소를 변경하지 않습니다.
